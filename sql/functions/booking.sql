...
) RETURNS TABLE (booking_id INT, booking_reference TEXT, total_amount NUMERIC) AS $$
DECLARE
new_booking_id INT;
...
SELECT generate_series(check_in_date, check_out_date - INTERVAL '1 day', '1 day') ::DATE
    LOOP
        IF NOT EXISTS (
            ...
            AND a.available_rooms >= total_rooms
        ) THEN
...
VALUES (new_booking_reference, user_id, property_id, room_type_id,
        ...
        guest_email, guest_phone, special_requests) RETURNING booking_id
INTO new_booking_id;
...
VALUES (new_booking_id, calculated_total, currency, payment_method,
        'completed', 'TXN-' || new_booking_reference, CURRENT_TIMESTAMP) RETURNING payment_id
INTO new_payment_id;
...
SELECT generate_series(new_check_in_date, new_check_out_date - INTERVAL '1 day', '1 day') ::DATE
        LOOP
            IF NOT EXISTS (
                ...
                AND a.available_date = date_iterator 
                AND a.available_rooms >= new_total_rooms
            ) THEN
                RETURN FALSE;
END IF;
END LOOP;
