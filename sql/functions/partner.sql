...
NOT EXISTS (
        ...
        AND p.partner_id = update_room_availability.partner_id
    ) THEN
...
FROM room_types rt
WHERE rt.room_type_id = update_room_availability.room_type_id ON CONFLICT (room_type_id, available_date)
        DO
UPDATE SET
...
SELECT p.property_id,
       ...
       COALESCE(ROUND(AVG(r.overall_rating), 1), 0)::NUMERIC AS avg_rating, p.created_at
FROM properties p
...
AS priority
    ...
    ORDER BY 
        CASE priority WHEN 'high' THEN 1 WHEN 'medium' THEN 2 ELSE 3
END,
        created_at DESC
    LIMIT limit_count;
END;
...
) RETURNS TABLE (partner_id INT, created_at TIMESTAMP) AS $
DECLARE
new_partner_id INT;
    new_created_at
TIMESTAMP;
...
VALUES (company_name, contact_email, crypt(password, gen_salt('bf', 8)),
        contact_person_first_name, contact_person_last_name,
        phone_number, target_country_id) RETURNING partner_id, created_at
...
INTO target_location_id
FROM locations
WHERE city_id = target_city_id LIMIT 1;
...
VALUES (partner_id, property_name, property_type, star_rating,
        ...
        contact_phone, contact_email) RETURNING property_id
...
VALUES (property_id, room_type_name, max_occupancy, max_adults,
        max_children, bed_configuration, base_rate, currency_code, total_rooms) RETURNING room_type_id
